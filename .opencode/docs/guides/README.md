# Guides

How-to guides for the OpenCode system.

## Contents

### Getting Started

- `user-installation.md` - Quick-start for new users
- `user-guide.md` - Comprehensive command workflows guide

### Component Development

- `component-selection.md` - Decision tree for creating commands, skills, or agents
- `creating-commands.md` - How to create commands
- `creating-skills.md` - How to create skills
- `creating-agents.md` - How to create agents

### Memory System

- `memory-setup.md` - Memory system setup and configuration
- `memory-troubleshooting.md` - Troubleshooting memory issues
- `learn-usage.md` - How to use the learn command

### Best Practices

- `context-loading-best-practices.md` - Context loading optimization
- `documentation-audit-checklist.md` - Documentation maintenance checklist
- `documentation-maintenance.md` - Keeping documentation up to date
- `phase-synchronization.md` - Phase synchronization patterns
- `tts-stt-integration.md` - Text-to-speech and speech-to-text integration

## Navigation

- [← Parent Directory](../README.md)
