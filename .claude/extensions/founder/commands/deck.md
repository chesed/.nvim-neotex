...
# Check for file path
elif echo "$ARGUMENTS" | grep -qE '^\.|^/|^~|\.md$|\.txt$|\.typ$'; then
...
- Run /implement {N} to generate the final Typst pitch deck
```
...
**Note**: Final Typst pitch deck is generated by `/implement`, not `/deck`.
...
/implement {N}          -> Executes plan, generates final Typst pitch deck
```
