...
**v002 Changes**:
- Removed blocked MCP tools documentation (Phase 3 in v001) - these are Lean-specific tools from ProofChecker and not applicable to this Neovim configuration repository
- Phase 3 now focuses solely on model enforcement
- Reduced total effort estimate from 17-26 hours to 14-21 hours
- Total phases reduced from 6 to 5

## Overview

This plan implements four prioritized features identified from the ProofChecker and Logos/Theory agent systems. The implementation is organized to maximize standalone value per phase, with optional Team Mode as a separate phase due to its experimental flag requirement. Context Index Schema (P2) provides high value without experimental features and is positioned early.
...
**Non-Goals**:
- Implementing Lean-specific features (lake-repair skill, blocked Lean tools)
...
- [ ] Update CLAUDE.md with model enforcement section
...
- `.claude/agents/planner-agent.md` - Add model field
- `.claude/CLAUDE.md` - Add model enforcement section

**Verification**:
- Agent frontmatter includes valid model field
- Model selection rationale is documented
- Task tool respects model field when spawning agents
...
**Phase 3**: Remove model field from frontmatter
